...
export default function PatientVisit({ patientId }: { patientId: string }) {
  ...
  // IPD details states
  const [ipdAdmissionDate, setIpdAdmissionDate] = useState(new Date().toISOString().split("T")[0]);
  const [ipdDischargeDate, setIpdDischargeDate] = useState("");
  const [ipdDailyExpense, setIpdDailyExpense] = useState("");

  const ipdStayDays = useMemo(() => {
    if (!ipdAdmissionDate || !ipdDischargeDate) return 0;
    const start = new Date(ipdAdmissionDate);
    const end = new Date(ipdDischargeDate);
  ...
  const ipdStayCost = assignmentType === "IPD" ? (ipdStayDays * (parseFloat(ipdDailyExpense) || 0)) : 0;
  const subtotal = consultationFee + registrationFee + testsFee + medicinesFee + extraCharge + customChargesTotal + ipdStayCost;
  ...
  if (state.success) {
    ...
    const billItems = [
      ...
      ...(assignmentType === "IPD" && ipdStayCost > 0 ? [{ name: `IPD Stay Charge (${ipdStayDays} days @ ₹${ipdDailyExpense}/day)`, value: ipdStayCost }] : []),
      ...
      <div className="max-w-3xl mx-auto my-10 p-8 bg-white border border-slate-200 rounded-xl shadow-lg print:border-none print:shadow-none print:my-0 print:p-0">
        ...
        <div className="grid grid-cols-2 gap-6 my-6 text-xs text-slate-600">
          ...
          <div className="text-right">
            ...
            <p>Care Mode: <span className="font-bold text-blue-700 uppercase">{assignmentType}</span></p>
            {assignmentType === "IPD" && (
              <p>Stay: {ipdAdmissionDate} to {ipdDischargeDate} ({ipdStayDays} Days)</p>
            )}
  ...
  return (
    <form action={formAction} className="max-w-4xl mx-auto p-6 space-y-6 print:hidden">
      ...
      <input type="hidden" name="ipdAdmissionDate" value={assignmentType === "IPD" ? ipdAdmissionDate : ""} />
      <input type="hidden" name="ipdDischargeDate" value={assignmentType === "IPD" ? ipdDischargeDate : ""} />
      <input type="hidden" name="ipdStayDays" value={assignmentType === "IPD" ? ipdStayDays.toString() : ""} />
      <input type="hidden" name="ipdDailyExpense" value={assignmentType === "IPD" ? ipdDailyExpense : ""} />
      ...
      <input type="hidden" name="extraCharge" value={extraCharge + customChargesTotal + ipdStayCost} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Section 1: OPD / IPD Mode Selection */}
          <section className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-6">
            ...
            {assignmentType === "OPD" ? (
              ...
              <div className="space-y-4">
                <div className="bg-blue-50/50 border border-blue-100/60 rounded-xl p-4 flex items-center gap-3">
                  <ShieldAlert className="w-5 h-5 text-blue-600 flex-shrink-0" />
                  <div>
                    <h4 className="font-bold text-blue-900 text-xs uppercase tracking-wide">Inpatient Department (IPD) Admission</h4>
                    <p className="text-[10px] text-blue-600 font-semibold mt-0.5">Please fill out stay dates and everyday hospitalization expenses below.</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-slate-500 font-bold text-[10px] uppercase mb-1">Admission Date</label>
                    <input
                      type="date"
                      value={ipdAdmissionDate}
                      onChange={(e) => setIpdAdmissionDate(e.target.value)}
                      className="w-full text-xs px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-200 focus:outline-none bg-slate-50/50"
                    />
                  </div>
                  <div>
                    <label className="block text-slate-500 font-bold text-[10px] uppercase mb-1">Discharge Date</label>
                    <input
                      type="date"
                      value={ipdDischargeDate}
                      onChange={(e) => setIpdDischargeDate(e.target.value)}
                      className="w-full text-xs px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-200 focus:outline-none bg-slate-50/50"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                  <div>
                    <label className="block text-slate-500 font-bold text-[10px] uppercase mb-1">Everyday Expense (₹/Day)</label>
                    <input
                      type="number"
                      placeholder="e.g. 1500"
                      value={ipdDailyExpense}
                      onChange={(e) => setIpdDailyExpense(e.target.value)}
                      className="w-full text-xs px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-200 focus:outline-none bg-slate-50/50"
                    />
                  </div>
                  <div className="flex flex-col justify-end">
                    <div className="p-3 bg-slate-50 border rounded-lg flex items-center justify-between text-xs">
                      <div>
                        <span className="text-slate-400 font-semibold text-[10px] uppercase block">Stay Duration</span>
                        <span className="font-bold text-slate-800">{ipdStayDays} Days</span>
                      </div>
                      <div className="text-right">
                        <span className="text-slate-400 font-semibold text-[10px] uppercase block">Total Daily Stay Cost</span>
                        <span className="font-extrabold text-blue-700">
                          ₹{((ipdStayDays || 0) * (parseFloat(ipdDailyExpense) || 0)).toLocaleString("en-IN")}
                        </span>
                      </div>
                    </div>
                  </div>
